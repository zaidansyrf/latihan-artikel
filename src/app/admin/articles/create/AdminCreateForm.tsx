    ...
    <main className="editor-page">
      <div className="editor-card" style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div className="editor-topbar">
          <div>
            <h2 style={{ fontSize: "20px", fontWeight: "bold" }}>Buat Artikel Baru (Admin)</h2>
            <p>Artikel akan otomatis diterbitkan ke publik.</p>
          </div>
        ...
        {/* PERHATIKAN: Tidak ada lagi onSubmit pemblokir form disini! */}
        <form action={saveAction} className="editor-body" style={{ display: "flex", flexDirection: "column", gap: "24px" }}> 
          <div className="image-upload-field">
            ...
            <label className="image-upload-box" style={{ position: "relative" }}>
              {/* KUNCI: Tambahkan name="image" di sini agar datanya terkirim ke server */}
              <input type="file" name="image" accept="image/*" onChange={handleImageChange} />
              {imagePreview ? (
                <>
                  <img src={imagePreview} alt="Preview" />
                  <button type="button" className="remove-image-btn" onClick={(e) => { e.preventDefault(); removeImage(); }}>&times;</button>
                </>
              ) : (<span>+ Klik untuk Upload Gambar</span>)}
            </label>
          </div>

          <input type="text" name="title" placeholder="Title here..." className="editor-title" required />

          <div className="editor-meta-row" style={{ display: "flex", gap: "16px", zIndex: 10 }}>
            <input type="text" name="author" placeholder="Nama Penulis" defaultValue="Admin YourStory" className="editor-image-input" style={{ flex: 1 }} />
          ...
          <textarea
            ...
            name="content"
            placeholder="Tulis artikel disini... (Blok teks lalu klik B, I, U di toolbar)"
            className="editor-content"
            required
            style={{ minHeight: "400px", padding: "16px", border: "1px solid #e5e7eb", borderRadius: "16px" }}
          />

          <button type="submit" className="editor-submit" style={{ alignSelf: "flex-start" }}>Publish Artikel</button>
        </form>
      </div>
    </main>
  );
